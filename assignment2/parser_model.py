...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        ### END YOUR CODE
        batch_size = w.size(0)
        w_one = w.view(-1).tolist()
        # print(w_one)
        x = torch.stack([self.embeddings[idx] for idx in w_one])
    ...
    def forward(self, w):
        ...
        hidden = torch.relu(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
